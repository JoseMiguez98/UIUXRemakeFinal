...
class Ranking extends Component {
    ...
    this.setState(prevState => (
      {
        ...
        hoverStars: prevState.stars.map((star, i) => (
            i <= index ? true : false
        ))
      }
    ));
  ...
  toggleStars(index) {
    notify.show(`Has votado ${index+1} estrella/s, gracias!`, 'custom', 2000, { background:'#111', text:'#fff' })
    this.setState(prevState => (
      {
        ...prevState,
        stars: prevState.stars.map((star, i) => (
            i <= index ? true : false
        ))
      }
    ));
  ...
  render() {
    ...
    stars = (!isHover ? this.state.stars : this.state.hoverStars);
    return (
      <div className="ranking-wrapper">
        <Notifications />
        <div className="ranking-wrapper__average">
          Valoración general: 4.5
        </div>
        <div className="ranking-wrapper__user">
          Deja tu valoración:
          { stars.map((star, index) => (
            <RankStar
              key={index}
              handleHover={() => this.toggleHover(index)}
              handleClick={() => this.toggleStars(index)}
              marked={star}
              width={25}
              height={25}
            />
